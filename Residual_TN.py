...
class Classifier():
    ...
    def generate_weight(self, name, shape):
        with tf.variable_scope(name, reuse=tf.AUTO_REUSE):
            weight = tf.random_normal(shape, mean=0, stddev=0.01)
    ...
    def build(self, image):
        ...
        for i in range(length - 2):
            with tf.variable_scope('c_middle%d' % i, reuse=tf.AUTO_REUSE):
                y = tf.einsum('nh,nj,hjk->nk', y, self.image[i + 1], self.weights[i+1]) + y
                self.outputs.append(y)
...
train_data = np.reshape(np.array(a).swapaxes(0, 1), [196, 55000, 1])
...
test_data = np.reshape(np.array(b).swapaxes(0, 1), [196, 10000, 1])
...
def train():
    ...
    with tf.variable_scope(tf.get_variable_scope(), reuse=tf.AUTO_REUSE):
        train_step = tf.train.AdamOptimizer(0.001).minimize(loss, var_list=T_vars)
